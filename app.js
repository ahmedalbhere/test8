...
let allBarbers = []; // متغير لحفظ جميع الحلاقين
...
// التحقق من وجود حجز سابق للعميل
...
async function barberSignup() {
    ...
    if (!name || !phone || !location || !city || !password || !confirmPassword) {
...
async function loadBarbers() {
    ...
    onValue(ref(database, 'barbers'), (snapshot) => {
        const barbers = snapshot.val() || {};
        allBarbers = Object.entries(barbers).map(([id, barber]) => ({
            id,
            ...barber
        }));
        
        filterBarbers(); // عرض الحلاقين مع التصفية
    });
}

// دالة تصفية الحلاقين حسب البحث
function filterBarbers() {
    const searchTerm = document.getElementById('barberSearch').value.trim().toLowerCase();
    const barbersList = document.getElementById('barbersList');
    
    if (!allBarbers.length) {
        barbersList.innerHTML = 'لا يوجد حلاقون متاحون';
        return;
    }
    ...
    if (searchTerm) {
        filteredBarbers = allBarbers.filter(barber => {
            const nameMatch = barber.name?.toLowerCase().includes(searchTerm);
            const cityMatch = barber.city?.toLowerCase().includes(searchTerm);
            return nameMatch || cityMatch;
        });
    }
    
    if (filteredBarbers.length === 0) {
        barbersList.innerHTML = 'لا توجد نتائج مطابقة للبحث';
        return;
    }
        ...
        barberCard.innerHTML = `
            <div class="barber-info">
                <div class="barber-header">
                    <div class="barber-avatar">${barber.name?.charAt(0) || 'ح'}</div>
                    <div class="barber-name">${barber.name || 'حلاق'}</div>
                </div>
                <div class="barber-status ${statusClass}">${statusText}</div>
                <div class="barber-details">
                    <div>المدينة: ${barber.city || 'غير متوفر'}</div>
...
async function bookAppointment(barberId, barberName) {
    ...
    // التحقق مما إذا كان العميل لديه حجز بالفعل
    ...
    try {
        ...
        // عرض الحجز الحالي
        showCurrentBooking();
        await loadBarbers(); // تحديث قائمة الحلاقين
...
function showCurrentBooking() {
    ...
    const bookingId = currentUser.booking.booking
